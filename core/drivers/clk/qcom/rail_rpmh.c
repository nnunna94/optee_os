...
/* A rail's own hlvl table and last-applied hlvl -- cx and mx never share a mapping. */
struct rpmh_rail_hlvl {
	uint32_t addr;
	uint16_t corners[RPMH_RAIL_HLVL_MAX];	/* ascending */
	size_t num_hlvls;
	uint32_t voted_hlvl;	/* hlvl applied here; INVALID if unknown/partial */
};
...
struct rpmh_rail {
	...
	struct rpmh_rail_hlvl mx;	/* .addr stays 0 if this chip has none */
	/* Refcount per corner requested, indexed via cx's table (always present, unlike mx). Demand tracking only -- both rails are still voted independently in rpmh_rail_apply(). */
...
/* Called once from clocks_register(); early_init() has already brought up cmd_db/rpmh by driver_init time. */
