...
/* clk->parent is the sole record of the currently-voted parent; rcg_set_rate() updates it on every switch. */
struct rcg_priv {
	...
	bool configured;	/* set_rate() has run at least once; enable()/disable() are no-ops until then */
};
...
/* Smallest config row >= @freq_hz; requires configs[] sorted ascending by freq_hz. */
...
/* Largest config row <= @freq_hz; same ascending-order requirement as find_freq(). */
static const struct clk_mux_config *
...
/* @src's SRC_SEL/parents[] index is per-RCG -- always via this RCG's own parent_map. */
...
/* Votes @pll via its own enabled_count directly -- clk_enable() would self-deadlock under lock_clk(). */
...
/* Latch a just-written CFG/M/N/D bank and wait for hardware to clear the update bit. */
...
/* Releases @old_parent's vote now that @new_parent's is live, then updates clk->parent. */
				    ...
				    unsigned long parent_rate __unused)
{
	...
	/* Disabled: rcg_enable() commits this, incl. the rail vote, once clk->parent is actually voted. */
	if (!enabled) {
		if (switching)
			clk->parent = new_parent;
	...
	if (raised)
		priv->corner = next_corner;
	else if (next_corner < curr_corner && !rail_vote(curr_corner, next_corner))
		priv->corner = next_corner;
...
/* Undoes rcg_disable()'s parking -- always runs on enable, since disable always leaves live hardware pointing at XO. */
...
/* No PLL/rail voting here -- hardware handles both autonomously once DFS_EN is set. */
...
/* Reads live hardware rather than trusting a cache: CMD_DFSR's SW_PERF_STATE once DFS is active, else CFG/M/N/D matched against configs[]. */
				       ...
				       unsigned long parent_rate __unused)
{
	...
	/* Disabled and not DFS-owned: live CFG reads back XO, parked there by rcg_disable(). */
	if (!priv->dfs && !clk_is_enabled(clk))
		return priv->rate;

	val = io_read32(priv->cmd_rcgr + QCOM_RCG_CFG_REG_OFFSET);
	mux_sel = (val & QCOM_RCG_CFG_SRC_SEL_FMSK) >> QCOM_RCG_CFG_SRC_SEL_SHFT;
	src_div = (val & QCOM_RCG_CFG_SRC_DIV_FMSK) >> QCOM_RCG_CFG_SRC_DIV_SHFT;
...
/* Reads live SRC_SEL rather than assuming index 0, since clk_register() seeds clk->parent from this. */
					...
					const struct clk_rcg_desc *desc)
{
	...
	return clk_regmap_va(desc->regmap, desc->cmd_rcgr_addr, &priv->cmd_rcgr);
}
...
/* Unwinds whatever clocks_register() already registered; each rcg is reached via clk_get_parent(). */
...
/* Copies out DFS-selectable rates paired with each row's DFS index -- never hands out a pointer into this driver's own config table. */
