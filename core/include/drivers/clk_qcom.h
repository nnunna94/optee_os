...
/* qcom has no secure DT, so a consumer acquires the clk by name instead of clk_dt_get_by_name(). */
...
/* Copies out DFS-selectable rates for @clk paired with each row's DFS index -- never hands out a pointer into this driver's own config table. */
